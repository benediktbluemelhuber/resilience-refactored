...
const Employees = () => {
  ...
  const handleButtonClick = (index, value) => {
    const newValues = [...valuesEmployees]
    newValues[index] = value
    setValues(newValues)
    localStorage.setItem('valuesEmployees', JSON.stringify(newValues))
  }
  ...
  return (<>
    <section className="mb-16">
				<div className={'text-gray-700 text-6xl font-bold mb-3'}>Mitarbeiter</div>
		</section>
    ...
    <div className="flex flex-col p-4 container center-left py-4">
      ...
      <ProgressBar value = {(9/9*100).toFixed(0)}/>
      </div>
    </div>
    </>
  )
